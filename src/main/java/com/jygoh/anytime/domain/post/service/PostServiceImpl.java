...
public class PostServiceImpl implements PostService {
    ...
    public Long createPost(PostCreateRequestDto requestDto, String token) {
        ...
        Post post = Post.builder().title(requestDto.getTitle()).content(requestDto.getContent())
            .author(member).mediaUrls(requestDto.getMediaUrls()).likeCount(0).commentCount(0)
            .reportCount(0).build();

        postRepository.save(post);

        List<PostHashtag> postHashtags = hashtagService.createHashtags(requestDto.getHashtags(),
            post);
        ...
        try {
            mediaService.moveMediaToPermanent(post.getMediaUrls());
            mediaService.deleteTempMedia(post.getMediaUrls());
        } catch (Exception e) {
            throw new RuntimeException("미디어 처리 중 오류가 발생했습니다: " + e.getMessage(), e);
        }
